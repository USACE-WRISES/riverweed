...
#'
#' \code{biomass.to.length.pod} calculates stem length of riverweed given biomass
...
#' biomass.to.length.pod(0.1, type=1)
#' 
#' #Result: Stem length of 43.11088 centimeters
#' biomass.to.length.pod(0.1, type=2)
#' 
#' #Result: Stem length of 14.82514 centimeters
#' biomass.to.length.pod(0.1, type=3)
#' 
#' #Result: Error message indicating incorrect model specification
#' biomass.to.length.pod(0.1, type=7)
#' 
#' #Result: Warning message indicating unrealistic biomass
#' #biomass.to.length.pod(1, type=1)
#' 
#' @export
biomass.to.length.pod <- function(afdm.g, type){
  #Error handling for invalid model specification
  if(!(type %in% seq(1,6))){stem.cm <- "Invalid model specification"}
